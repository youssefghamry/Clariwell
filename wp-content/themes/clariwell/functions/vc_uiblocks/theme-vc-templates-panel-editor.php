...
class Clariwell_Vc_Templates_Panel_Editor {
	/**
	 * @since 4.4
	 * @var bool
	 */
	protected $invictus_templates = false;
	/**
	 * @since 4.4
	...
	public function addTemplatesTab( $data ) {
		$newCategory = array(
			'category'        => 'invictus_templates',
			'category_name'   => esc_html__( 'Clariwell Templates', 'clariwell' ),
			'category_weight' => 1,
	...
	protected function get_template_categories() {
		...
		$categories = array(
			'all'				=> esc_html__( 'All', 'clariwell' ),
			'content-blocks'		=> esc_html__( 'Content Blocks', 'clariwell' ),
			'icon-boxes'			=> esc_html__( 'Icon Boxes', 'clariwell' ),
            'image-boxes'		=> esc_html__( 'Image Boxes', 'clariwell' ),
            'blog'				=> esc_html__( 'Blog', 'clariwell'),
			'portfolio'			=> esc_html__( 'Portfolio', 'clariwell' ),
			'counters'			=> esc_html__( 'Counters', 'clariwell' ),			
			'clients'			=> esc_html__( 'Clients', 'clariwell' ),
			'pricing-tables'		=> esc_html__( 'Pricing Tables', 'clariwell' ),
			'team-members'			=> esc_html__( 'Team Members', 'clariwell' ),
			'testimonial'			=> esc_html__( 'Testimonials', 'clariwell' ),
			'tabs'			=> esc_html__( 'Tabs', 'clariwell' ),
			'accordions'			=> esc_html__( 'Accordions', 'clariwell' ),
			'contact'			=> esc_html__( 'Contact', 'clariwell' ),
			'call-to-action'		=> esc_html__( 'Call to Action', 'clariwell' ),
	...
	public function renderTemplateBlock( $category ) {
		...
		if ( 'invictus_templates' === $category['category'] ) {

			$category['output'] = '<div class="vc_col-md-2 invictus-sorting-container">';
			$category['output'] .= $this->get_template_categories();
			$category['output'] .= '</div>';


			$category['output'] .= '
			<div class="vc_column vc_col-sm-12 invictus-templates-container">
	...
	function renderTemplateWindow( $template_name, $template_data ) {
		...
		if ( 'invictus_templates' === $template_data['type'] ) {
			return $this->renderTemplateWindowClariwellTemplates( $template_name, $template_data );
		}
	 ...
	 * @return string
	 */
	public function renderTemplateWindowClariwellTemplates( $template_name, $template_data ) {
		ob_start();
		...
		$preview_template_title = esc_attr( 'Preview template', 'clariwell' );
		$add_template_title = esc_attr( 'Add template', 'clariwell' );
...
HTML;
	...
	public function renderBackendTemplate() {
		...
		if ( ! isset( $template_id, $template_type ) || '' === $template_id || '' === $template_type ) {
			die( 'Error: Vc_Clariwell_Templates::renderBackendTemplate:1' );
		}
	 ...
	 * @param $templates
	 *
	 * vc_filter: vc_load_Invictus_templates_limit_total - total items to show
	 *
	 * @return array
	 */
	...
	public function getAllTemplates() {
		...
		$data = array();
		$invictus_templates = $this->getTemplates();
		$category_templates = array();
		foreach ( $invictus_templates as $template_id => $template_data ) {
			$category_templates[] = array(
				'unique_id' => $template_id,
				'name' => $template_data['name'],
				'type' => 'invictus_templates',
	 ...
	 * To modify you should use add_filter('vc_load_invictus_templates','your_custom_function');
	 ...
	 *          ...
	 *      );
	 * Also see filters 'vc_load_invictus_templates_panels' and 'vc_load_invictus_templates_welcome_block' to modify
	 * templates in panels tab and/or in welcome block. vc_filter: vc_load_invictus_templates - filter to override
	...
	public function loadDefaultTemplates() {
		...
		if ( ! is_array( $this->invictus_templates ) ) {
			$this->invictus_templates = $this->allTemplates();
		}

		return $this->invictus_templates;
	}
	...
	public function getDefaultTemplate( $template_index ) {
		...
		if ( ! is_numeric( $template_index ) || ! is_array( $this->invictus_templates ) || ! isset( $this->invictus_templates[ $template_index ] ) ) {
			return false;
		}

		return $this->invictus_templates[ $template_index ];
	}
	...
	public function addDefaultTemplates( $data ) {
		if ( is_array( $data ) && ! empty( $data ) && isset( $data['name'], $data['content'] ) ) {
			if ( ! is_array( $this->invictus_templates ) ) {
				$this->invictus_templates = array();
			}
			$this->invictus_templates[] = $data;
	...
	public function renderTemplateListItem( $template ) {
		$name = isset( $template['name'] ) ? esc_html( $template['name'] ) : esc_html( __( 'No title', 'clariwell' ) );
...
HTML;
		$output .= '<div class="invictus-template-preview"><img src="' . esc_url( $template_image ) . '" alt="' . esc_attr( $name ) . '" width="300" height="200" /></div>';
		$output .= apply_filters( 'vc_templates_render_template', $name, $template );
		$output .= '<span class="invictus-template-categories">' . esc_html( $template_sort_name ) . '</span>';
		$output .= <<<HTML
						</div>
