...
$class_to_filter = 'vc_progress_bar insignia-progress-bar wpb_content_element';
...
foreach ( $graph_lines_data as $line ) {
	...
	$output .= '<div class="vc_general vc_single_bar' . ( ( isset( $line['color'] ) && 'custom' !== $line['color'] ) ?
		...
		. '">';
	// For secondary line, added by Veented
	
	if (strpos($line['label'],'%%') !== false) {
	
		$labels = explode("%%", $line['label']);
	    $output .= '<small class="vc_label vc_label_with_secondary"' . $line['txtcolor'] . '><span class="progress-bar-value">' . esc_textarea($labels[0]) . $unit . '<span class="progress-bar-secondary-label">' . esc_textarea($labels[1]) . '</span></span></small>';
